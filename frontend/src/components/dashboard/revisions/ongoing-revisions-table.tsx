...
interface OngoingRevisionsTableProps {
  revisions: Revision[];
  refreshRevisions: () => void;
}

export function OngoingRevisionsTable({
  revisions,
  refreshRevisions,
}: OngoingRevisionsTableProps) {
  ...
  const handleVerify = (revision: Revision) => {
    ...
    console.log("Continue revision:", revision.id);
    alert(`Continuar revisión del proyecto: ${revision.project?.name}`);
  };

  if (!revisions || revisions.length === 0) {
    return (
      <p className="text-gray-500">
        No se han encontrado revisiones en curso.
      </p>
    );
  }
              ...
              <TableCell className="text-center">
                ...
                >
                  ...
                  <span className="sr-only">
                    Continuar revisión de {revision.project?.name}
                  </span>
                </Button>
              </TableCell>
            </TableRow>
